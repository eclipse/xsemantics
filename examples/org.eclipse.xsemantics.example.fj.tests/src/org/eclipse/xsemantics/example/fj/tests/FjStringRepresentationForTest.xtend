...
class FjStringRepresentationForTest extends FjStringRepresentation {
	...
	def dispatch customRep(New exp) {
		'''new «exp.type.classref.name»(''' + exp.args.map [
			it.string
		].join(", ") + ''')'''
	}

	def dispatch customRep(Selection exp) {
		'''«exp.receiver.string».''' + switch (exp.message) {
			Field: {
				exp.message.name
			}
			Method: {
				exp.message.name + "(" + exp.args.map[it.string].join(", ") + ")"
			}
		}
	}
