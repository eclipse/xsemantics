...
class LambdaEditorTest extends AbstractEditorTest {
	...
	def void testLambdaEditorContentsAsModel() {
		"x".assertEquals(
			"lambda x. x".openEditorWithFile.abstractionFromEditor.param.name
		)
	}

	@Test
	def void testInferAndModifyAbstractionType() throws Exception {
		"lambda x. x".inferAbstractionTypeAndExpect("a")
	}

	@Test
	def void testInferAndModifyComposeAbstractionType() {
		"lambda f . lambda g. lambda x. (f (g x))".inferAbstractionTypeAndExpect("a -> b")
	}

	@Test
	def void testInferAndModifyAbstractionWithExplicitType() {
		"lambda x : int -> string. x".inferAbstractionTypeAndExpect("a")
	}
	...
	def void testInferAndModifyNotTypableAbstractionWithType() {
		...
		"lambda x : int -> string. x x".inferAbstractionTypeAndExpect("int -> string")
	}

	@Test
	def void testInferAndModifyTerm() {
		"lambda f . lambda g. lambda x. (f (g x))".inferTypeAndExpect(
	...
	def void testInferAndModifyWrongTerm() throws Exception {
		// since the type cannot be inferred the original type is not changed
		"lambda f . lambda g. lambda x. (f (f f))".inferTypeAndExpect("lambda f . lambda g. lambda x. (f (f f))")
	}

	def private inferAbstractionTypeAndExpect(String contents, String expected) {
		expected.assertEquals(
			contents.modifyAbstractionWithInferredType.param.type.rep
		)
	}
...
//	}
	def private void modifyAbstractionWithInferredType(XtextEditor editor) {
		editor.document.modifyAbstractionWithInferredType
	}

	def private void modifyAbstractionWithInferredType(IXtextDocument xtextDocument) {
		lambdaTermModifier.modifyAbstractionWithInferredType(xtextDocument);
	}

	def private void modifyTermWithInferredType(XtextEditor editor) {
