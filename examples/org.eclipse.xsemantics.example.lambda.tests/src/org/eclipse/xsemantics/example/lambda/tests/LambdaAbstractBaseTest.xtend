...
abstract class LambdaAbstractBaseTest {
	@Inject protected LambdaValidator validator

	@Inject protected Injector injector

	@Inject protected extension ParseHelper<Program>
	...
	def protected Program getLambdaProgram(String program) {
		program.parse
	}
	...
	def <T> void assertFailure(Result<T> result) {
		if (!result.failed)
			Assert::assertTrue(
				"should have failed, instead got: " + result.value.string + "\n" + trace.traceAsString,
				result.failed
			);
	}
	...
	def <T> void assertResultAsString(Result<T> result, String expected) {
		if (result.failed) {
			Assert::assertFalse(
				"should not have failed: " + result.ruleFailedException.failureTraceAsString,
				result.failed
			)
		}
