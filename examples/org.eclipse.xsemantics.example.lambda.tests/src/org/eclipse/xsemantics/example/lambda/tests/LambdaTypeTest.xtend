...
class LambdaTypeTest extends LambdaAbstractBaseTest {
	...
	def void testExplicitVariableType() {
		...
		assertType(abstraction.environmentForParam, abstraction.term.variable, "int", "")
	}

	@Test
	def void testImplicitVariableType() {
		val abstraction = "lambda x . x".abstraction
		assertType(abstraction.environmentForParam, abstraction.term.variable, "X1", "")
	}
	...
	def void testAbstractionExplicitVariableType() {
		assertAbstractionType("lambda x : int . x", "int -> int", "")
	}

	@Test
	def void testAbstractionImplicitVariableType() {
		val abstraction = "lambda x . x".abstraction
		assertType(abstraction, "a -> a", "")
	}

	@Test
	def void testAbstractionWithArithmetics() {
		assertType("lambda x . -x".abstraction, "int -> int", "X1=int")
	}

	@Test
	def void testApplicationType() {
		assertAbstractionType("lambda x .lambda y . x y", "(a -> b) -> a -> b", "X1=(X3 -> X4), X2=X5, X3=X5")
	}

	@Test
	def void testApplicationType2() {
		assertAbstractionType("lambda x .lambda y . y x", "a -> (a -> b) -> b", "X1=X5, X2=(X3 -> X4), X3=X5")
	}

	@Test
	def void testCompose() {
		assertAbstractionType("lambda f . lambda g. lambda x. (f (g x))", "(a -> b) -> (c -> a) -> c -> b",
			"X1=(X4 -> X5), X2=(X6 -> X7), X3=X8, X4=X9, X6=X8, X7=X9")
	}

	@Test
	def void testCompose2() {
		assertAbstractionType("lambda f . lambda g. lambda x. (g (f x))", "(a -> b) -> (b -> c) -> a -> c",
			"X1=(X6 -> X7), X2=(X4 -> X5), X3=X8, X4=X9, X6=X8, X7=X9")
	}

	@Test
	def void testCompose3() {
		assertAbstractionType("lambda x . lambda y. lambda z. x z (y z)", "(a -> b -> c) -> (a -> b) -> a -> c",
   ...
   failed: ApplicationType: [f <- X1, g <- X2, x <- X3] |- subst{X1=(X4 -> X5), X4=X8, X5=X9, X6=X8, X7=X9} |> (f (f f)) : Type
	...
	def void assertParamType(CharSequence prog, String expectedType) {
		assertResultAsString(
			system.paramtype(prog.abstraction.param),
			expectedType
		)
	}
	...
	def void assertType(RuleEnvironment env, Term term, String expectedType, String expectedSubsts) {
		assertResultAsStringBeautifier(
			system.type(env, null, substitutions, term),
			expectedType
		)
