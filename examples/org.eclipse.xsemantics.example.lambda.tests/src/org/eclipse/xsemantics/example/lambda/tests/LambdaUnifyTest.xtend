...
class LambdaUnifyTest extends LambdaAbstractBaseTest {
	...
	def void testUnifyArrowTypes2() {
		assertUnify(
			lambdaUtils.createFreshArrowType,
			lambdaUtils.createArrowType(lambdaUtils.createFreshArrowType, lambdaUtils.createFreshArrowType),
			"((X3 -> X4) -> (X5 -> X6))",
			"X1=(X3 -> X4), X2=(X5 -> X6)"
		)
	}
	...
	def void testUnifyArrowTypesWithBasicTypes() {
		assertUnify(
			...
			lambdaUtils.createArrowType(
				lambdaUtils.createArrowType(lambdaUtils.createIntType, lambdaUtils.createStringType),
				lambdaUtils.createFreshArrowType
			),
			"((int -> String) -> (X3 -> X4))",
	...
	def void testUnifyArrowTypesWithBasicTypes2() {
		assertUnify(
			lambdaUtils.createArrowType(
				lambdaUtils.createFreshArrowType,
				lambdaUtils.createArrowType(lambdaUtils.createStringType, lambdaUtils.createIntType)
			),
			lambdaUtils.createArrowType(
				lambdaUtils.createArrowType(lambdaUtils.createIntType, lambdaUtils.createStringType),
				lambdaUtils.createFreshArrowType
