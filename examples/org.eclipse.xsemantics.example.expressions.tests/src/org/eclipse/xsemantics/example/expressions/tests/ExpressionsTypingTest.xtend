...
class ExpressionsTypingTest extends ExpressionsAbstractBaseTests {
...
failed: cannot type !('abc')
 StringType cannot be assigned to BooleanType'''
	...
	def void assertResultAndTrace(CharSequence program, int variableIndex, String expectedResult,
		...
		if (expectedResult !== null) {
			...
			if (!result.failed) {
				Assert::fail(
					"unexpected success: " + trace.traceAsString
				)
			}
			if (expectedTrace !== null)
				assertEqualsStrings(
